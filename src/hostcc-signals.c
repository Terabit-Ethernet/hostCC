...
void update_iio_rd_occ_ctl_reg(void){
  ...
  uint64_t msr_num;
	msr_num = IIO_MSR_PMON_CTL_BASE + (0x20 * STACK) + IIO_RD_COUNTER_OFFSET;
...
void sample_iio_rd_occ_counter(int c){
  ...
  uint32_t low = 0;
	uint32_t high = 0;
	msr_num = IIO_MSR_PMON_CTR_BASE + (0x20 * STACK) + IIO_RD_COUNTER_OFFSET;
...
void update_iio_wr_occ_ctl_reg(void){
	//program the desired CTL register to read the corresponding CTR value
  uint64_t msr_num;
	msr_num = IRP_MSR_PMON_CTL_BASE + (0x20 * STACK) + IIO_WR_COUNTER_OFFSET;
...
void sample_iio_wr_occ_counter(int c){
  ...
  uint32_t low = 0;
	uint32_t high = 0;
	msr_num = IRP_MSR_PMON_CTR_BASE + (0x20 * STACK) + IIO_WR_COUNTER_OFFSET;
  rdmsr_on_cpu(c,msr_num,&low,&high);
