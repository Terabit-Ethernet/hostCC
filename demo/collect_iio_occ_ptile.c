...
void dump_ptile(){
    // float desired_percentile = ptile;
    // float result = find_percentile((counter%LOG_SIZE)-1, desired_percentile);
    // printf("%d",result);

    std::vector<float> data;
    for(int i=0;i<counter%LOG_SIZE;i++){
        data.push_back((float)(LOG[i].s_avg_occ_longterm));
    }
...
int main(){
    ...
    while(1){
        ...
        update_log(cpu);
        counter++;
        time_from_checkpoint_ns = ((rdtscp() - last_checkpoint) * 10) / 33;
        if(time_from_checkpoint_ns >= 1e9){
            // printf("Here: %ld\n",(counter-1));
            dump_ptile();
            // main_exit();
            return 0;
