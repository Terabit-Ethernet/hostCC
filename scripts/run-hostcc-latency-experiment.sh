...
help()
{
    echo "Usage: run-hostcc-latency-experiment
...
LONG=home:,server:,client:,num_servers:,num_clients:,server_intf:,client_intf:,exp:,txn:,MTU:,size:,ddio:,cpu_mask:,lat_app_core:,mlc_cores:,target_iio_thresh:,target_pcie_thresh:,help
OPTS=$(getopt -a -n run-hostcc-latency-experiment --options $SHORT --longoptions $LONG -- "$@")
...
#post-run cleanup
fin_hostcc $setup_dir/logs/$exp-LATRUN-$j
cleanup
done
